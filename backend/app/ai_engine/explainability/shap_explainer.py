        ...
        contributing_factors.append(
            {
                ...
                "weight": 0.85,
                "detail": "File movement detected across device chain",
            }
        )
...
def explain_ransomware_detection(
    ...
    if "encryption_activity" in indicator_summary:
        reasons.append(
            f"File encryption detected ({indicator_summary['encryption_activity']} files)"
        )
        contributing_factors.append(
            {
                "factor": "encryption",
...
def _shap_explanation(
    ...
    try:
        ...
        reasons = [
            f"{f['feature']} = {f['value']:.2f} ({f['direction']} anomaly score by {f['importance']:.3f})"
...
def _rule_based_explanation(
    ...
    reasons = [
        f"{f['feature']} = {f['value']:.2f} (elevated)"
