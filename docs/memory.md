...
- [x] **D9(d):** Build design-token CSS/Tailwind config — **DONE.** `tailwind.config.js` + `globals.css` now have: all 8 core color tokens (§1.1/§1.2), disabled/archived color `#5A6478`, 4 light-mode tokens (§1.3: paper-50, ink-900-print, rule-line, print-accent), 3 font families (§2.1), 7 type scale entries (§2.2: display-lg/sm, body-lg/sm, label, mono-md/sm). Google Fonts import covers all required weights.
