...
**Last updated:** 2026-08-27 (session 6 continued) by agent
...
| Phase 3 — Dashboard GUI | 🟨 In Progress | ⬜ | NextAuth ✅ runtime-verified; React Query ✅ all pages migrated (049ab75); TS clean. Zod ✅ (7ab8861). Design tokens ✅ (a82a8a1). Timeline ✅ (067b7c6). Correlation graph ✅ (6e5a562 + 83cbed5) — color deviations fixed. Puppeteer-verified 11/11. RBAC ✅ (cea2baa) — backend API-layer enforcement + frontend role gates. Remaining: root layout, live RBAC testing (Docker not running), Scenario 1/2 validation |
...
- [x] RBAC-aware UI (viewer/investigator/admin) per phases.md Phase 3 — DONE (cea2baa). Backend: `require_case_access` dependency (ADMIN sees all, others must be assigned), self-registration locked to VIEWER role (prevents privilege escalation), 6 GET endpoints now enforce case-access check, case-update ownership check added. Frontend: `useRole()` hook + `<RoleGate>` component in `lib/rbac.tsx`, cases list/case detail/anomaly panel action buttons hidden for viewers. Test script: `scripts/test-rbac.ps1` (Docker required for live testing).
...
- **State:** D1(a) NextAuth runtime-verified. D2(b) React Query migration complete (049ab75). D3(c) Zod DONE (7ab8861). D9(d) Design tokens DONE (a82a8a1). D14(e) Timeline DONE (067b7c6). D15(f) Correlation graph DONE (6e5a562 + 83cbed5) — color deviations fixed, puppeteer-verified 11/11. D16(g) RBAC DONE (cea2baa) — backend API-layer enforcement (require_case_access, self-registration locked to VIEWER, case-update ownership check) + frontend role gates (useRole hook, RoleGate component, action buttons hidden for viewers). TS clean. `__mock__` route removed from timeline page.
- **Next concrete step:** D17(h) — Build real root layout with sidebar (case name), navigation, auth wrapper (login/logout, role badge). Requires reading design.md for layout spec.
