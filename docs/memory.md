...
- **Module:** Phase 3 — D16(g) RBAC **code-complete, needs live verification** (next up before D17(h))
- **State:** D1(a) NextAuth runtime-verified. D2(b) React Query migration complete (049ab75). D3(c) Zod DONE (7ab8861). D9(d) Design tokens DONE (a82a8a1). D14(e) Timeline DONE (067b7c6). D15(f) Correlation graph DONE (6e5a562 + 83cbed5) — puppeteer-verified 11/11. D16(g) RBAC **code-complete only** (cea2baa) — backend API-layer enforcement + frontend role gates implemented, but NOT runtime-verified (Docker was not running). Must run test-rbac.ps1 + browser check before marking verified. TS clean.
- **Next concrete step:** Start Docker → run test-rbac.ps1 → fix failures → browser-verify viewer role gates → mark D16(g) ✅ verified → then D17(h) root layout.
